...
public class Read_XLS {
	...
	//XSSF = xlsx format excell files
	private XSSFWorkbook wb = null;
	private XSSFSheet ws = null;
	List<String> list = new ArrayList<String>();
	//This is a constructor. fileName contains file name of the sheet
	//relativePath is relative path of project where the file is
	public Read_XLS(String fileName, String relativePath) {
	this.fileName = fileName;
	//Try-catch in case anything is wrong when accessing the file and putting info from excel sheet to the website
	try {
	...
	//This method is to get information of the rows
	...
	//This method is to get information of the columns
	...
	public String retrieveToRunFlag(String wsName, String colName, String rowName) {
		//ignoring the null value of the sheet
		int sheetIndex = wb.getSheetIndex(wsName);
		if (sheetIndex == -1)
			return null;
		else {
		 ...
		 for(int i=0; i<rowNum; i++) {
			 ...
			 if(row == null) {
				 ...
				 if(cell == null) {
					 ...
					 //if your cell value is not string than convert it into string data
		 ...
		 for(int i = 0; i< rowNum - 1; i++) {
			 ...
			 for (int j=0; j < colNum; j++) {
				 ...
				 if(cell == null) {
					 ...
					 //Need to check for latest update this one is deprecated
					 cell.setCellType(cell.CELL_TYPE_STRING);
					 String value = cellToString(cell);
					 data[i][j] = value;
				 }
